...
import { BookOpen, Users, Lock, Star, CheckCircle } from "lucide-react"

interface LearningMethodsProps {
...
export function LearningMethods({
  ...
  onGroupLearning,
  studentLevel = 1,
}: LearningMethodsProps) {
  const [selectedMethod, setSelectedMethod] = useState("")
  const [description, setDescription] = useState("")
  const [showDialog, setShowDialog] = useState(false)

  const handleUseMethod = () => {
    if (selectedMethod && description.trim().length >= 15) {
      // Level-Überprüfung hinzufügen
      const requiredLevel = getLearningMethodRequiredLevel(selectedMethod)
      if (studentLevel < requiredLevel) {
        alert(
          `❌ Du benötigst Level ${requiredLevel}, um "${selectedMethod}" zu verwenden! Dein aktuelles Level: ${studentLevel}`,
        )
        return
      }
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            {unlockedMethods.map((method) => (
              <div key={method} className="p-4 border rounded-lg hover:border-blue-300 transition-colors bg-white">
                  ...
                  <DialogTrigger asChild>
                    <Button
                      ...
                      onClick={() => {
                        ...
                        if (studentLevel < requiredLevel) {
                          alert(
                            `❌ Du benötigst Level ${requiredLevel}, um "${method}" zu verwenden! Dein aktuelles Level: ${studentLevel}`,
                          )
                          return
                        }
                  ...
                  <DialogContent>
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <Textarea
                          id="description"
                          placeholder="z.B. Ich habe die Feynman-Methode verwendet, um Photosynthese zu erklären. Dabei habe ich..."
                          value={description}
                        ...
                        <p className="text-xs text-gray-500 mt-1">
                          Beschreibe einfach, was du gemacht hast (mindestens 15 Zeichen)
                        </p>
                      </div>
